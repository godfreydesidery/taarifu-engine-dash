...
class AdminUserService {
  ...
  createAdminUser(request) {
    return this.http.post(`${this.API_URL}/admin/v1/users/admin-users`, request);
  }
  // Update Admin User by UID
  updateAdminUser(uid, request) {
    return this.http.put(`${this.API_URL}/admin/v1/users/admin-users/uid/${uid}`, request);
  }
  // Get Admin User by UID
  getAdminUserByUid(uid) {
    return this.http.get(`${this.API_URL}/admin/v1/users/admin-users/uid/${uid}`);
  }
  // Get Admin User by Username
  getAdminUserByUsername(username) {
    return this.http.get(`${this.API_URL}/admin/v1/users/admin-users/username/${username}`);
  }
  // Get Admin User by Email
  getAdminUserByEmail(email) {
    return this.http.get(`${this.API_URL}/admin/v1/users/admin-users/email/${email}`);
  }
  // Get All Admin Users with Pagination
  getAllAdminUsers(page = 0, size = 10, sortBy = 'createdAt', sortDir = 'desc') {
    ...
    return this.http.get(`${this.API_URL}/admin/v1/users/admin-users`, {
      params
    });
  }
  // Activate Admin User
  activateAdminUser(uid) {
    return this.http.put(`${this.API_URL}/admin/v1/users/admin-users/uid/${uid}/activate`, {});
  }
  // Deactivate Admin User
  deactivateAdminUser(uid) {
    return this.http.put(`${this.API_URL}/admin/v1/users/admin-users/uid/${uid}/deactivate`, {});
  }
  // Suspend Admin User
  suspendAdminUser(uid) {
    return this.http.put(`${this.API_URL}/admin/v1/users/admin-users/uid/${uid}/suspend`, {});
  }
  // Get Admin User Count
  getAdminUserCount() {
    return this.http.get(`${this.API_URL}/admin/v1/users/admin-users/count`);
  }
  // Check Username Availability
  isUsernameAvailable(username) {
    return this.http.get(`${this.API_URL}/admin/v1/users/admin-users/check-username/${username}`);
  }
  // Check Email Availability
  isEmailAvailable(email) {
    return this.http.get(`${this.API_URL}/admin/v1/users/admin-users/check-email/${email}`);
  }
  // Get Admin User Stats (if available)
  getAdminUserStats() {
    return this.http.get(`${this.API_URL}/admin/v1/users/admin-users/stats`);
  }
  // Search Admin Users (if available)
  searchAdminUsers(query, page = 0, size = 10, sortBy = 'createdAt', sortDir = 'desc') {
    ...
    return this.http.get(`${this.API_URL}/admin/v1/users/admin-users/search`, {
      params
    });
  }
  // Get Admin Users by Status
  getAdminUsersByStatus(status, page = 0, size = 10, sortBy = 'createdAt', sortDir = 'desc') {
    const params = new _angular_common_http__WEBPACK_IMPORTED_MODULE_0__.HttpParams().set('page', page.toString()).set('size', size.toString()).set('sortBy', sortBy).set('sortDir', sortDir);
    return this.http.get(`${this.API_URL}/admin/v1/users/admin-users/status/${status}`, {
      params
    });
  }
  // Reset Admin User Password (generates new password and sends via email)
  resetAdminUserPassword(uid, request = {}) {
    ...
    return this.http.put(`${this.API_URL}/admin/v1/users/admin-users/uid/${uid}/reset-password`, resetRequest);
  }
  // Change Admin User Password (requires current password verification)
  changeAdminUserPassword(uid, request) {
    return this.http.put(`${this.API_URL}/admin/v1/users/admin-users/uid/${uid}/change-password`, request);
  }
  // Force Password Reset (admin can force a user to reset their password)
  forcePasswordReset(uid, request = {}) {
    // Generate a secure password if not provided
    const newPassword = request.newPassword || this.generateSecurePassword();
    const resetRequest = {
      newPassword,
    ...
    return this.http.put(`${this.API_URL}/admin/v1/users/admin-users/uid/${uid}/force-password-reset`, resetRequest);
  }
  /**
  ...
  static {
    this.ɵfac = function AdminUserService_Factory(__ngFactoryType__) {
      return new (__ngFactoryType__ || AdminUserService)(_angular_core__WEBPACK_IMPORTED_MODULE_1__["ɵɵinject"](_angular_common_http__WEBPACK_IMPORTED_MODULE_0__.HttpClient));
    };
  }
  static {
